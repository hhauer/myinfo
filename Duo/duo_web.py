...
ERR_AKEY = 'ERR|The application secret key passed to sign_request() must be at least %s characters.'.format(AKEY_LEN)
...
def _sign_vals(key, vals, prefix, expire):
    ...
    val = '|'.join(vals + [ exp ])
    b64 = base64.b64encode(bytes(val, 'UTF-8'))
    cookie = '{}|{}'.format(prefix, b64.decode('UTF-8'))

    sig = _hmac_sha1(bytes(key, 'UTF-8'), bytes(cookie, 'UTF-8'))
    return '{}|{}'.format(cookie, sig)

def _parse_vals(key, val, prefix, ikey):
    ts = int(time.time())
    u_prefix, u_b64, u_sig = val.split('|')

    sig = _hmac_sha1(key, '{}|{}'.format(u_prefix, u_b64))
